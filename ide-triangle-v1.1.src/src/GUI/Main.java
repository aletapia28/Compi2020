...
public class Main extends javax.swing.JFrame {
    ...
    /**
     * Checks if the file has been changed - used to enable/disable the "Save"
     * button.
     */
    ...
    /**
     * Checks if there are any open documents. Disables some buttons and menu
     * options if no document is open.
     */
    ...
    /**
     * Creates a new document frame and inserts it into the desktop pane, enabling
     * its buttons and menu options.
     *
    ...
    private void initComponents() {
        ...
        compileMenuItem
                .setIcon(new javax.swing.ImageIcon(getClass().getResource("/GUI/Icons/iconTriangleCompile.gif")));
    ...
    private void openMenuItemActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_openMenuItemActionPerformed
        ...
        if (chooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                JOptionPane.showMessageDialog(null, "An error occurred while trying to open the specified file",
    ...
    private void compileMenuItemActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_compileMenuItemActionPerformed
        ...
        if (((FileFrame) desktopPane.getSelectedFrame()).getPreviouslySaved()) {
            ...
            if (compiler.compileProgram(desktopPane.getSelectedFrame().getTitle())) {
                ...
                // disassembler.Disassemble(desktopPane.getSelectedFrame().getTitle().replace(".tri",
                // ".tam"));
                ((FileFrame) desktopPane.getSelectedFrame())
                        .setTree((DefaultMutableTreeNode) treeVisitor.visitProgram(compiler.getAST(), null));
    ...
    private void saveMenuItemActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_saveMenuItemActionPerformed
            ...
            if (chooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
                if (chooser.getSelectedFile().exists()) {
                    overwrite = (JOptionPane.showConfirmDialog(this,
                            chooser.getSelectedFile().getName() + " already exists.\nWould you like to replace it?",
        ...
        if (overwrite) {
            try {
                ...
                JOptionPane.showMessageDialog(null, "An error occurred while trying to save the specified file",
                        "Error", JOptionPane.ERROR_MESSAGE);
            }
            ...
            ((FileFrame) desktopPane.getSelectedFrame())
                    .setPreviousSize(((FileFrame) desktopPane.getSelectedFrame()).getSourcePaneText().length());
    ...
    /**
     * Runs every time a key is pressed in the text editor frame, thus determining
     * if the file contents have changed, activating the "Save" button and menu
     * option.
     */
    KeyAdapter delegateSaveButton=new KeyAdapter(){public void keyReleased(java.awt.event.KeyEvent evt){checkSaveChanges();((FileFrame)desktopPane.getSelectedFrame()).UpdateRowColNumbers();}};

    MouseListener delegateMouse=new MouseListener(){public void mouseClicked(java.awt.event.MouseEvent evt){((FileFrame)desktopPane.getSelectedFrame()).UpdateRowColNumbers();}
    ...
    /**
     * Every time a frame is focused/activated, some buttons can be enabled (e.g.
     * "Save", "Cut", "Copy", "Paste").
     */
    public void internalFrameActivated(InternalFrameEvent evt){checkPaneChanges();((FileFrame)desktopPane.getSelectedFrame()).UpdateRowColNumbers();}

    /**
     * Every time a frame is closed, some buttons can be disabled (e.g. "Save",
     * "Cut", "Copy", "Paste").
     */
    public void internalFrameClosed(InternalFrameEvent evt){checkPaneChanges();}

    /**
     * Before closing a frame, checks if the file has not been saved yet.
     */
    public void internalFrameClosing(InternalFrameEvent evt){if(((FileFrame)desktopPane.getSelectedFrame()).hasChanged()){if(JOptionPane.showConfirmDialog(null,"Do you want to save the changes to "+desktopPane.getSelectedFrame().getTitle()+"?","Save",JOptionPane.YES_NO_OPTION)==JOptionPane.YES_OPTION)saveMenuItemActionPerformed(null);}}

    // Required by interface - not implemented
    public void internalFrameDeactivated(InternalFrameEvent evt){}public void internalFrameDeiconified(InternalFrameEvent evt){}public void internalFrameIconified(InternalFrameEvent evt){}public void internalFrameOpened(InternalFrameEvent evt){}};

    /**
     * Used to redirect the console output - writes in the "Console" panel.
     */
    ActionListener delegateConsole=new ActionListener(){public void actionPerformed(ActionEvent e){while(output.peekQueue())((FileFrame)desktopPane.getSelectedFrame()).writeToConsole(output.readQueue());}};

    /**
     * Used to redirect the console output - writes in the "TAM Code" pane.
     */
    ActionListener delegateTAMCode=new ActionListener(){public void actionPerformed(ActionEvent e){while(output.peekQueue())((FileFrame)desktopPane.getSelectedFrame()).writeToTAMCode(output.readQueue());}};

    /**
     * Used to redirect the console input - enables the "Console Input" text box.
     */
    ActionListener delegateInput=new ActionListener(){public void actionPerformed(ActionEvent e){((FileFrame)desktopPane.getSelectedFrame()).setInputEnabled(true);}};

    /**
     * Used to redirect the console input - writes the user input in the console.
     */
    ActionListener delegateEnter=new ActionListener(){public void actionPerformed(ActionEvent e){((FileFrame)desktopPane.getSelectedFrame()).setInputEnabled(false);input.addInput(((FileFrame)desktopPane.getSelectedFrame()).getInputFieldText()+"\n");((FileFrame)desktopPane.getSelectedFrame()).clearInputField();}};

    /**
     * Used to control running programs - only one TAM program can be run at once
     */
    ActionListener delegateRun=new ActionListener(){public void actionPerformed(ActionEvent e){runMenuItem.setEnabled(true);buttonRun.setEnabled(true);compileMenuItem.setEnabled(true);buttonCompile.setEnabled(true);}};
    // </editor-fold>
    ...
    // ">
    /**
     * ClipboardOwner Class Internal clipboard management class, uses the default
     * system clipboard.
     */
